  ...
  return (
    <nav className="w-full bg-white  py-4 px-20 lg:px-2  top-0 z-50 fixed">
      <div className="container mx-auto flex items-center  justify-between">
        {/* Lottie Logo */}
        <div className="w-64 flex items-center justify-center h-10 gap-2">
          <Lottie animationData={logoAnimation} loop className="w-10 h-20" />
          <span className="whitespace-nowrap text-lg font-semibold  md:block">
            Chapters & Co.
          </span>
        </div>

        {/* Desktop Navigation */}
        <ul className="hidden md:flex flex-wrap justify-center gap-4 text-lg font-medium">
          {navLinks.map((link, index) => (
            <li key={link.key} className="flex items-center hover:text-[#2972b6]">
              <Link
                to={link.path}
                className={`${
                  location.pathname === link.path ? "text-[#2972b6]" : "text-black"
